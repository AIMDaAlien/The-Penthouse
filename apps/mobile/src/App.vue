...
import { formatRecoveryCode, type ChatMessage } from '@penthouse/contracts';
import {
  deletePushToken,
...
import { useMessageSendQueue } from './composables/useMessageSendQueue';
...
import { TEST_NOTICE_SUMMARY, TEST_NOTICE_TITLE } from './testNotice';
...
let ensureSocketConnectedRef: (() => void) | null = null;
...
const realtime = useRealtimeConnection({
  ...
  touchChat,
  setChatUnreadCount,
  bumpChatUnreadCount: chatState.bumpChatUnreadCount,
  isChatMuted,
  queueInAppToast,
  scheduleIncomingMessageNotification,
  markMessageDelivered,
...
const {
  ...
  realtimeDiagnostics,
  realtimeState,
  resetState: resetRealtimeState,
  restartSocketConnection,
  scheduleMarkSelectedChatRead,
  setAppActive: setRealtimeAppActive,
  setPresenceStatus,
  wireSocketHandlers
} = realtime;

ensureSocketConnectedRef = ensureSocketConnected;
...
function resetWorkspaceState(): void {
  ...
  clearInAppToasts();
  resetChatState();
  resetRealtimeState(session.value?.user.id ?? null);
...
function setAppActive(nextValue: boolean): void {
  ...
  setRealtimeAppActive(nextValue);
  if (nextValue && !wasActive) {

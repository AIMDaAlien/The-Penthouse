<!--
  V7-09 Threshold — five baseline effects + form folds-swings on Y-axis hinge when key turns.
-->
<script lang="ts">
	...
	function handlePress() {
		if (!form.canSubmit || form.loading) return;
		swung = true;
		setTimeout(() => { swung = false; }, 1400);
		void form.submit();
	}
</script>
...
<div class="root">
	...
	<article class="article">
		<header class="art-head" use:typeReveal={{ delay: 0 }}>
			...
			<p class="issue-line"><em>A door, hinging open.</em></p>
		</header>
		<div class="art-body">
			<h1 class="headline" use:typeReveal={{ delay: stagger(1) }}>The form<br/><em>is the hinge.</em></h1>
			<p class="lead" use:typeReveal={{ delay: stagger(2) }}>
				<span class="dropcap" aria-hidden="true">T</span><span class="dropcap-h">T</span>he key turns. The room turns with it. You arrive somewhere a moment before you have asked for it.
			</p>
		</div>
	</article>
	...
	<aside class="glass-col" use:typeReveal={{ delay: stagger(2) }}>
		<div class="glass">
			<header class="g-head"><span class="g-kicker"><em>Sign-in</em></span><h2 class="g-title">Turn it open.</h2></header>
			<div class="tabs">
			...
			{#key form.mode}
				<form class="form" class:swung in:fly={tabSwitchIn} out:fly={tabSwitchOut} onsubmit={(e) => e.preventDefault()}>
					...
					<KeyButton loading={form.loading} disabled={form.loading || !form.canSubmit} label="Open the door" onpress={handlePress} />
				</form>
			{/key}
		</div>
	</aside>
...
<style>
	...
	.glass-col { position: relative; z-index: 1; align-self: center; perspective: 1400px; }
	...
	.form { display: flex; flex-direction: column; gap: 18px; transform-origin: left center; transform-style: preserve-3d; transition: transform 1100ms cubic-bezier(0.22, 1, 0.36, 1), opacity 800ms ease-out; }
	.form.swung { transform: rotateY(-72deg); opacity: 0.45; }
	...
	@media (prefers-reduced-motion: reduce) { .form { transition: none !important; } .dropcap { animation: none !important; } .tab { transition: none !important; } }
</style>

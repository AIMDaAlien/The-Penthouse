<!--
  V7-01 Aurora — canonical V7. Cool periwinkle aurora orbs, all five baseline
  effects (drop cap pulse, caret bloom, floating labels, sweep underline, key turn).
  Additional animation: aurora orbs orbit in slow figure-eight on infinite cycle.
-->
<script lang="ts">
...
<div class="root">
	...
	<article class="article">
		<header class="art-head" use:typeReveal={{ delay: 0 }}>
			...
			<p class="issue-line"><em>Spring issue,</em> Year MMXXVI.</p>
		</header>
		<div class="art-body">
			<h1 class="headline" use:typeReveal={{ delay: stagger(1) }}>A house with<br/><em>eleven floors,</em><br/>and one door.</h1>
			<p class="lead" use:typeReveal={{ delay: stagger(2) }}>
				<span class="dropcap" aria-hidden="true">T</span><span class="dropcap-h">T</span>he lift waits at the lobby. A small slip is placed on the desk, the light above it dim and considered. Write your name. Pretend not to look at the others.
			</p>
		</div>
	</article>
	...
	<aside class="glass-col" use:typeReveal={{ delay: stagger(2) }}>
		<div class="glass">
			<header class="g-head">
				<span class="g-kicker"><em>Sign-in</em></span>
				<h2 class="g-title">Take the lift.</h2>
			</header>
			...
			{#key form.mode}
				<form class="form" onsubmit={form.submit} in:fly={tabSwitchIn} out:fly={tabSwitchOut}>
					...
					{#if form.mode === 'register'}
						...
						<label class="check">
							<input type="checkbox" bind:checked={form.acceptedNotice} disabled={form.loading} />
...
<style>
	.root {
		...
		background: oklch(0.11 0.018 285);
		padding: 56px;
		overflow: hidden;
		isolation: isolate;
	}
	...
	.dropcap {
		...
		padding-top: 0.06em;
		animation: dcpulse 2400ms ease-in-out infinite;
		transform-origin: center;
	}
	.dropcap-h { font-family: var(--font-display); visibility: hidden; }
	@keyframes dcpulse {
		0%, 100% { opacity: 1; transform: scale(1); text-shadow: 0 0 14px oklch(0.69 0.140 285 / 0.22); }
		50%      { opacity: 0.86; transform: scale(0.985); text-shadow: 0 0 32px oklch(0.69 0.140 285 / 0.50); }
	}

	.glass-col { position: relative; z-index: 1; align-self: center; }
	.glass {
		padding: 32px 32px 30px;
		background: oklch(0.21 0.025 285 / 0.42);
		backdrop-filter: blur(28px) saturate(150%);
		-webkit-backdrop-filter: blur(28px) saturate(150%);
		border: 1px solid oklch(1 0 0 / 0.10);
		border-radius: 24px;
		box-shadow:
			0 30px 70px oklch(0 0 0 / 0.50),
	...
	.tab.active { background: var(--p-accent); color: var(--p-bg); }
	...
	@media (prefers-reduced-motion: reduce) {
		.aura, .dropcap { animation: none !important; }

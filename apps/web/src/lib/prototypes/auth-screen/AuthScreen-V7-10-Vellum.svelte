<!--
  V7-10 Vellum — five baseline effects + animated grain overlay + orchestrated load
  reveal sequence (wordmark -> headline -> lead -> card -> form rows).
-->
<script lang="ts">
...
<div class="root">
	...
	<article class="article">
		<header class="art-head" use:typeReveal={{ delay: 0 }}>
			...
			<p class="issue-line"><em>Set on vellum, by hand.</em></p>
		</header>
		<div class="art-body">
			<h1 class="headline" use:typeReveal={{ delay: 220, distance: 14 }}>The page<br/><em>has a tooth.</em></h1>
			<p class="lead" use:typeReveal={{ delay: 440, distance: 12 }}>
				<span class="dropcap" aria-hidden="true">T</span><span class="dropcap-h">T</span>he surface holds a faint grain, light shifting across it like a candle considered. The pause before you sign has its own atmosphere.
			</p>
		</div>
	</article>

	<aside class="glass-col" use:typeReveal={{ delay: 660, distance: 16 }}>
		<div class="glass">
			<header class="g-head" use:typeReveal={{ delay: 820 }}><span class="g-kicker"><em>Sign-in</em></span><h2 class="g-title">A grained welcome.</h2></header>
			<div class="tabs" use:typeReveal={{ delay: 980 }}>
			...
			{#key form.mode}
				<form class="form" onsubmit={form.submit} in:fly={tabSwitchIn} out:fly={tabSwitchOut}>
					...
					<div use:typeReveal={{ delay: 1320, distance: 8 }}>
...
<style>
	...
	.grain { position: absolute; inset: 0; z-index: 1; pointer-events: none; background-image: var(--tex); background-size: 420px 420px; mix-blend-mode: overlay; opacity: 0.55; animation: grainShift 2400ms steps(8) infinite; }
	...
	.article { position: relative; z-index: 2; display: flex; flex-direction: column; gap: 36px; max-width: 640px; }
	...
	.glass-col { position: relative; z-index: 2; align-self: center; }
	...
	@media (prefers-reduced-motion: reduce) { .grain { animation: none !important; } .dropcap { animation: none !important; } .tab { transition: none !important; } }
</style>

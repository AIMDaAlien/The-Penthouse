<!--
  V7-06 Tug — five baseline effects + magnetic key button (cursor pulls it within ~130px).
-->
<script lang="ts">
...
<div class="root">
	...
	<article class="article">
		<header class="art-head" use:typeReveal={{ delay: 0 }}>
			...
			<p class="issue-line"><em>The handle, leaning closer.</em></p>
		</header>
		<div class="art-body">
			<h1 class="headline" use:typeReveal={{ delay: stagger(1) }}>The door<br/><em>meets you halfway.</em></h1>
			<p class="lead" use:typeReveal={{ delay: stagger(2) }}>
				<span class="dropcap" aria-hidden="true">T</span><span class="dropcap-h">T</span>he key knows when a hand is nearby. It will turn toward you with a small, polite tug, like a host shifting a chair.
			</p>
		</div>
	</article>
	...
	<aside class="glass-col" use:typeReveal={{ delay: stagger(2) }}>
		<div class="glass">
			<header class="g-head"><span class="g-kicker"><em>Sign-in</em></span><h2 class="g-title">The key, attentive.</h2></header>
			<div class="tabs">
			...
			{#key form.mode}
				<form class="form" onsubmit={form.submit} in:fly={tabSwitchIn} out:fly={tabSwitchOut}>
					...
					<KeyButton loading={form.loading} disabled={form.loading || !form.canSubmit} magnetic onpress={() => form.submit()} />
				</form>
			{/key}
		</div>
	</aside>

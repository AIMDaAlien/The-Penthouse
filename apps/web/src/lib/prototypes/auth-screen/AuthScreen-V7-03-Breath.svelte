<!--
  V7-03 Breath — five baseline effects + italic words breathe + aurora orbs breathe in sync.
-->
<script lang="ts">
...
<div class="root">
	...
	<article class="article">
		<header class="art-head" use:typeReveal={{ delay: 0 }}>
			...
			<p class="issue-line"><em class="breath">A slow, attentive breath.</em></p>
		</header>
		<div class="art-body">
			<h1 class="headline" use:typeReveal={{ delay: stagger(1) }}>The room <em class="breath">inhales,</em><br/>then waits.</h1>
			<p class="lead" use:typeReveal={{ delay: stagger(2) }}>
				<span class="dropcap" aria-hidden="true">T</span><span class="dropcap-h">T</span>he aurora keeps a measured rhythm, like a host counting under their breath. <em class="breath">Match it</em>, and the door will know.
			</p>
		</div>
	</article>
	...
	<aside class="glass-col" use:typeReveal={{ delay: stagger(2) }}>
		<div class="glass">
			<header class="g-head"><span class="g-kicker"><em class="breath">Sign-in</em></span><h2 class="g-title">A measured entry.</h2></header>
			<div class="tabs">
			...
			{#key form.mode}
				<form class="form" onsubmit={form.submit} in:fly={tabSwitchIn} out:fly={tabSwitchOut}>
					...
					{#if form.mode === 'register'}
						...
						<label class="check"><input type="checkbox" bind:checked={form.acceptedNotice} disabled={form.loading} /><span><em class="breath">An alpha.</em> Data may be wiped without warning.</span></label>
					{/if}
...
<style>
	...
	.aura { position: absolute; border-radius: 50%; filter: blur(100px); animation: auraBreath 5800ms ease-in-out infinite; }
	.a-a { width: 620px; height: 620px; background: oklch(0.69 0.140 285 / 0.55); top: -120px; right: 18%; animation-delay: 0ms; }
	.a-b { width: 520px; height: 520px; background: oklch(0.50 0.110 285 / 0.45); bottom: -160px; left: -80px; animation-delay: 1900ms; }
	@keyframes auraBreath { 0%,100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.10); opacity: 0.78; } }
	...
	.issue-line em { color: var(--p-accent); }
	...
	.headline em { color: var(--p-accent); }
	.lead { font-family: var(--font-display); font-size: 1.15rem; line-height: 1.65; color: var(--p-text-2); max-width: 52ch; position: relative; }
	.lead em { color: var(--p-accent); }

	.breath { font-style: italic; display: inline-block; transform-origin: center; animation: italicBreath 3600ms ease-in-out infinite; }
	.breath:nth-of-type(even) { animation-delay: 700ms; }
	@keyframes italicBreath { 0%,100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.012); opacity: 0.92; text-shadow: 0 0 14px oklch(0.69 0.140 285 / 0.32); } }
	...
	@media (prefers-reduced-motion: reduce) { .aura, .breath, .dropcap { animation: none !important; } .tab { transition: none !important; } }
</style>

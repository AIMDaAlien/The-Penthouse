<!--
  V7-07 Ripple — five baseline effects + keystroke ripples (small periwinkle dots that fade).
-->
<script lang="ts">
...
<div class="root">
	...
	<article class="article">
		<header class="art-head" use:typeReveal={{ delay: 0 }}>
			...
			<p class="issue-line"><em>Every letter leaves a wake.</em></p>
		</header>
		<div class="art-body">
			<h1 class="headline" use:typeReveal={{ delay: stagger(1) }}>A name typed,<br/><em>a ripple kept.</em></h1>
			<p class="lead" use:typeReveal={{ delay: stagger(2) }}>
				<span class="dropcap" aria-hidden="true">T</span><span class="dropcap-h">T</span>he desk has a soft echo. Each letter you press is acknowledged, then forgotten, in that order.
			</p>
		</div>
	</article>
	...
	<aside class="glass-col" use:typeReveal={{ delay: stagger(2) }}>
		<div class="glass">
			<header class="g-head"><span class="g-kicker"><em>Sign-in</em></span><h2 class="g-title">The desk listens.</h2></header>
			<div class="tabs">
			...
			{#key form.mode}
				<form class="form" onsubmit={form.submit} in:fly={tabSwitchIn} out:fly={tabSwitchOut}>
					<AuthInput label="Username" bind:value={form.username} autocomplete="username" autocapitalize="none" disabled={form.loading} ripple />
					<AuthInput label="Password" type="password" bind:value={form.password} autocomplete={form.mode === 'login' ? 'current-password' : 'new-password'} disabled={form.loading} ripple />
					{#if form.mode === 'register'}
						<AuthInput label="Confirm password" type="password" bind:value={form.confirmPassword} autocomplete="new-password" disabled={form.loading} ripple />
						<AuthInput label="Display name" bind:value={form.displayName} autocomplete="name" disabled={form.loading} optional ripple />
						<AuthInput label="Invite code" bind:value={form.inviteCode} autocapitalize="characters" spellcheck={false} disabled={form.loading} upper ripple />
...
<style>
	...
	@media (prefers-reduced-motion: reduce) { .dropcap { animation: none !important; } .tab { transition: none !important; } }
</style>

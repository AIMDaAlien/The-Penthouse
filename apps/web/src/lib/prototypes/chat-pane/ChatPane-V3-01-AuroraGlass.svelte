...
<div class="chat-pane v3-variant-1">
  <div class="blobs">
...
<style>
  ...
  .b1 { top: 15%; left: 10%; width: 80%; height: 80%; background: var(--blob1); transform: rotate(-15deg); animation: float1 15s infinite alternate ease-in-out; }
  .b2 { bottom: -20%; right: -10%; width: 90%; height: 70%; background: var(--blob2); transform: rotate(15deg); animation: float1 20s infinite alternate-reverse ease-in-out; opacity: 0.8; }
  .b3 { top: 40%; left: 30%; width: 60%; height: 60%; background: var(--blob3); mix-blend-mode: multiply; animation: float1 18s infinite alternate ease-in-out; }

  @keyframes float1 {
    0% { transform: translate(0, 0) scale(1); }
    100% { transform: translate(25px, -28px) scale(1.1); }
  }

<script lang="ts">
	// ChatPane V5-05 — Charcoal Rust (T-D7 dark)
	// Self-contained prototype. No external imports.

	const me = { id: 'me', name: 'You', avatar: 'https://i.pravatar.cc/150?u=ren', status: 'online' as const };
	const them = { id: 'them', name: 'Kai', avatar: 'https://i.pravatar.cc/150?u=kai', status: 'online' as const };
	...
	const messages: Msg[] = [
		{ id: '1', sender: them, content: 'the forge is still warm', time: '19:42' },
		{ id: '2', sender: them, content: 'i tempered the last blade an hour ago', time: '19:43' },
		{ id: '3', sender: me, content: 'how did the grain look?', time: '19:44' },
		{ id: '4', sender: them, content: 'tight. like river sand.', time: '19:45', reactions: [{ emoji: '⚒', count: 1 }] },
		{ id: '5', sender: me, content: 'good. that is what we need for the order.', time: '19:46' },
		{ id: '6', sender: them, content: 'twelve more by friday. i can manage.', time: '19:47' },
		{ id: '7', sender: me, content: 'i will bring the steel in the morning', time: '19:48' },
		{ id: '8', sender: them, content: 'early. before the heat.', time: '19:49' },
		{ id: '9', sender: me, content: 'before the heat', time: '19:50', reactions: [{ emoji: '◇', count: 1 }] },
		{ id: '10', sender: them, content: 'sleep well, smith', time: '19:51' },
	];
...
<div class="pane">
	...
	<header class="head">
		<div class="head-left">
			...
			<div class="head-meta">
				...
				<span class="head-eyebrow">N° 07 / ONLINE</span>
			</div>
		</div>
	</header>
	...
	<div class="messages">
		<div class="day-divider">
			...
			<span class="day-label">TONIGHT</span>
			<span class="day-line"></span>
		</div>
...
<style>
	.pane {
		/* Charcoal Rust — deep charcoal with warm rust accent */
		--p-bg:          oklch(0.18 0.010 35);
		--p-surface:     oklch(0.23 0.015 35);
		--p-surface-2:   oklch(0.28 0.020 35);
		--p-text:        oklch(0.93 0.012 35);
		--p-text-2:      oklch(0.80 0.025 35);
		--p-muted:       oklch(0.65 0.050 35);
		--p-muted-2:     oklch(0.50 0.040 35);
		--p-accent:      oklch(0.69 0.140 35);
		--p-accent-soft: oklch(0.69 0.140 35 / 0.16);
		--p-accent-edge: oklch(0.69 0.140 35 / 0.36);
		--p-secondary:   oklch(0.78 0.090 35);
		--p-line:        oklch(0.78 0.090 35 / 0.12);
		--p-line-2:      oklch(0.78 0.090 35 / 0.22);
		--p-warning:     oklch(0.62 0.070 35);
		--p-success:     oklch(0.68 0.140 145);
		--p-away:        oklch(0.82 0.130 80);
		--p-offline:     oklch(0.55 0.020 35);
	...
	.time {
		...
		color: var(--p-muted); opacity: 0.75; white-space: nowrap;
	}
	...
	.row.own .bub { background: var(--p-accent-soft); border-color: var(--p-accent-edge); }
	.bub-text {
	...
	.composer-wrap { position: relative; z-index: 1; padding: 14px 32px 22px; background: var(--p-surface); }
	.composer {
